...
def is_product_query(query: str) -> bool:
    ...
    Returns True only if query contains product-related keywords.
    """
...
def calculate_relevance_score(query: str, metadata: Dict[str, Any]) -> float:
    """
    Calculate a relevance score based on query-metadata matching.
...
def is_relevant_result(query: str, doc_metadata: Dict[str, Any], 
    ...
    if vector_score is not None:
        try:
            ...
            # If score is high (far distance), likely irrelevant
            if score_val > 0.95:
                return False, f"Vector distance too high: {score_val:.3f}"
            # If score is very low (close distance) and relevance check passes
    ...
    try:
        ...
        if not is_product_query(query):
            ...
            return JSONResponse({
                ...
                "debug": {
                    "relevance": False,
                    "reason": "Not a product query"
                }
            })
        
        print("✓ Query is product-related")
        
        # Step 2: Retrieve from vector store with score
        ...
        if not results_with_scores:
            ...
            return JSONResponse({
                ...
                "debug": {
                    "relevance": False,
                    "reason": "No vector store results"
                }
            })
